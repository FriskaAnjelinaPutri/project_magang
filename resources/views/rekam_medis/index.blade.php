...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
    ...
    <a href="{{ route('rekam-medis.cetak') }}" target="_blank" class="px-5 py-2.5 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-bold transition-all shadow-md hover:shadow-lg flex items-center transform hover:-translate-y-0.5">
        ...
        <table class="w-full text-left border-separate border-spacing-0">
            <thead>
                <tr class="bg-gray-50/90">
                    ...
                    <th class="py-3.5 px-4 text-xs font-bold text-gray-500 uppercase tracking-wider border-b border-gray-200/80">Tindakan / Resep Obat</th>
                </tr>
            </thead>
            <tbody>
                @forelse ($rekamMedis as $rm)
                    <tr class="odd:bg-white even:bg-gray-50/50 hover:bg-orange-50/40 transition-colors">
                        ...
                        <td class="py-4 px-4 border-b border-gray-100/80">
                            <div class="text-sm text-gray-800">
                                <span class="font-semibold text-gray-600">Tindakan:</span> {{ $rm->tindakan }}<br>
                                <span class="font-semibold text-green-600">Resep:</span> {{ $rm->resep_obat }}
                            </div>
                        </td>
                    </tr>
                @empty
