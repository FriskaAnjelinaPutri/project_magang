...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 px-2 mt-4 gap-4">
    <div>
        <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">Pendaftaran</h1>
        <p class="text-sm text-gray-500 mt-1">Monitoring seluruh data pendaftaran dan antrean pasien.</p>
    </div>
    <a href="{{ route('pendaftaran.create') }}" class="btn-gradient font-bold py-2.5 px-6 rounded-full shadow-lg transition-all text-sm flex items-center gap-2">
                ...
                <tbody>
                    @forelse ($pendaftaran ?? [] as $row)
                        ...
                        <tr class="border-b border-gray-100/50 hover:bg-white/40 transition-colors">
                            <td class="py-4 px-4 text-sm font-bold text-orange-500 text-center">{{ $row->id_pendaftaran }}</td>
                            ...
                            <td class="py-4 px-4 min-w-[180px]">
                                <form method="POST" action="{{ route('pendaftaran.update', $row->id_pendaftaran ?? $row->id) }}" class="inline-block w-full max-w-[220px]">
                                    @csrf
                                    @method('PUT')
                                    <label class="sr-only">Ubah status pendaftaran</label>
                                    <select name="status" onchange="this.form.submit()"
                                        ...
                                        <option value="dipanggil" {{ $statusRaw === 'dipanggil' ? 'selected' : '' }}>Dipanggil</option>
                                        <option value="diproses" {{ $statusRaw === 'diproses' ? 'selected' : '' }}>Diproses</option>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="py-10 text-center text-sm font-medium text-gray-500">Belum ada data pendaftaran terbaru.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
